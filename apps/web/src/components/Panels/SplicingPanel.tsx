...
export const SplicingPanel = ({ image }: { image: any }) => {
    ...
    const run = async () => {
        ...
        setError("");
        try {
            const res = await axios.post('/api/forensic/splicing/noiseprint', {
                image_path: image.fullPath
            });
            if (res.data.error) setError(res.data.error);
            else setResult(res.data); // Expect result_url or similar
        } catch (err: any) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#323232] p-4">
            <h3 className="text-gray-300 font-bold border-b border-gray-600 pb-2 mb-2">Noiseprint Analysis</h3>
